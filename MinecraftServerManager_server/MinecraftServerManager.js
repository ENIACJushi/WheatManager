    ...
    if((message != undefined) && (message != false)) {
        ...
        if(synNBT){
            ...
            if(pl.setNbt(playerNBT)) {
                pl.refreshItems();
                logger.info(`Success: Local NBT data of ${pl.realName} has been changed.`);
            }
            else {
                logger.info(`Failed: Local NBT data of ${pl.realName} has not been changed`);
        ...
        if(synTags){
            if(message.tags != [] ) {
                ...
                logger.info(`Success: Local tag data of ${pl.realName} has been changed.`);
            }
        }
        ...
        if(synScores){
            ...
            if(score_.score != []) {
                ...
                logger.info(`Success: Local scoreboard data of ${pl.realName} has been changed.`);
            }
        }
        ...
        if(synLLMoney){
            ...
            logger.info(`Success: Local LLMoney data of ${pl.realName} has been changed.`);
        }
...
mc.listen('onServerStarted', function() {
    ...
    if(WS.status != 0) {
        logger.info('stauts:', WS.connect(WSHostURL), WS.status);
...
setInterval(() => {
    if(WS.status == 2){
        ...
        logger.info('Trying reconnect to WebSocket server');
        logger.info('stauts:', WS.connect(WSHostURL), WS.status);
